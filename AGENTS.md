...
- `{"cmd":"reset"}` -> `AT#RESET`.
- Alerting is fully frontend-owned. The backend forwards each `#EVTDETECT` as a logged detection event (the snapshot `events` carry the structured `unit_a/unit_b/value/threshold/device_ts` fields) and does not compute any alarm state — there is no `crossing_alert` field, no auto-reset, and no `ack` message. The frontend raises a crossing alarm from detection events, clears it ~4s after the last detection for that pair, and dismissal hides the banner until the crossing ends (the alarm clears), after which a fresh crossing of the same pair alarms again.
